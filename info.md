# ewxSync — Internal documentation
...
Bulky reference material lives under `docs/`.
...
- `reqs.md` — goals, requirements, and the bug backlog (migrated from the former `goals.md`). Check it before assuming a quirk is intended behavior: known open bugs are listed there, not fixed silently. Detail files live under `reqs/`.
- `todo.md` — the development plan: the work items not yet built, each referencing the `reqs.md` ID it advances.
- `docs/eventworx API analysis.md` — reverse-engineered notes on EWX request shapes and filter values.
...
| Question | Look in |
|---|---|
| How does an EWX endpoint/filter/status behave? | [docs/eventworx API analysis.md](docs/eventworx%20API%20analysis.md) — endpoints, filter language, Solr↔response field map, status vocabularies, per-docType label tables, verified findings (project entity, REST API, docSubType). |
| Is this quirk a known bug? | [reqs.md](reqs.md) — goals, requirements, bug backlog. |
| What's planned / on the work list? | [todo.md](todo.md) — the development plan (incl. the "Watching Eventworx" items). |
...
| What are the raw EWX status identifiers? | [docs/eventworx job-status store.txt](docs/eventworx%20job-status%20store.txt) — committed excerpt of the frontend `EventWorx.store.lookups.JobStatus` store (status `identifier` → German `displayText`). A static snapshot; re-grep `app.pretty.js` for the live version. |
...
Active status sets (derived from Eventworx's own filter requests, see [docs/eventworx API analysis.md](docs/eventworx%20API%20analysis.md)):
...
- **Lifecycle**: create-only. No archive, move, or delete when a project leaves the target set — channel relocation on completion is a manual step (see `reqs.md`).
...
- **All transitions announced**: every status change for every docType is posted (no curated subset). Observed `status` values per docType are catalogued in [docs/eventworx API analysis.md](docs/eventworx%20API%20analysis.md), and each is mapped to a German predicate phrase in `STATUS_PHRASES`.
...
Bulky reference material lives under `docs/`:
- [docs/eventworx API analysis.md](docs/eventworx%20API%20analysis.md) — reverse-engineered EWX request shapes, filter language, status vocabularies, per-docType label tables, and verified findings.
- [docs/eventworx job-status store.txt](docs/eventworx%20job-status%20store.txt) — committed excerpt of the frontend `JobStatus` lookup store (status identifier → German label).
